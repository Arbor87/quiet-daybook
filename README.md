...
一个以“今天”为中心的私人工作台：日历式 Todo、每日想法、自然语言记账和月度预算。数据只保存在当前浏览器中，登录仅用于进入应用。
...
任务、想法、开销、预算和余额只保存在当前设备的 IndexedDB，不会上传或在设备间同步。百炼 API Key 和“密钥库”也只保存在当前设备。

## 数据与 AI

- 电脑和手机各自保存独立数据，不会自动同步；建议定期导出 JSON 备份。
